...
class SensorSpecs:
    ...
    def __post_init__(self):
        ...
        if self.measuring_height <= 0:
            raise ValueError("measuring_height must be positive")
...
class FrequencyResponse:
    ...
    def __init__(self, sensor_specs: SensorSpecs, atm_conditions: AtmConditions):
        """Initialize with sensor and atmospheric parameters."""
        if not isinstance(sensor_specs, SensorSpecs):
            raise ValueError("sensor_specs must be a SensorSpecs instance")
        if not isinstance(atm_conditions, AtmConditions):
            raise ValueError("atm_conditions must be an AtmConditions instance")
        ...
        # Validate parameters
        if any(x <= 0 for x in [
            self.specs.path_length,
            self.specs.measuring_height,
            self.specs.sampling_frequency
        ]):
            raise ValueError("Length and frequency parameters must be positive")

        # Derived parameters
        self.f_nyquist = self.specs.sampling_frequency / 2
        self.normalized_freq = None
        self.transfer_funcs = {}
    ...
    def calculate_correction_factor(
        ...
        Args:
            stability: Atmospheric stability (z/L)
            integration_method: Numerical integration method ('trapz', 'midpoint')

        Returns:
            Correction factor (ratio of true to measured flux)
        """
        # Validate stability range
        if abs(stability) > 5.0:
            raise ValueError("Stability value must be between -5.0 and 5.0")
        ...
        valid_methods = ['trapz', 'midpoint']
        if integration_method not in valid_methods:
            raise ValueError(
                f"Invalid integration method. Must be one of: {valid_methods}"
            )
        ...
        if integration_method == 'trapz':
            ...
            x_mid = (self.normalized_freq[1:] + self.normalized_freq[:-1]) / 2
            numerator = np.sum(Co[1:] * np.diff(self.normalized_freq))
            denominator = np.sum(
                Co[1:] * H[1:] * np.diff(self.normalized_freq)
            )

        # Guard against division by zero
        if denominator == 0:
            raise ValueError("Zero denominator in correction factor calculation")

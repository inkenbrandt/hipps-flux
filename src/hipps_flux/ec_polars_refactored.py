...
class CalcFlux:
    ...
    def run_irga(self, df, *, rename_map: dict | None = None, ts_col: str | None = None, lazy: bool = False) -> pd.Series:
        """
...
import pandas as pd  # optional compat
        ...
        # --- Normalise input to pandas.DataFrame, allow optional renaming and timestamp casting ---
        import pandas as _pd
        import pathlib as _plpath

        if isinstance(df, (str, bytes, _plpath.Path)):
            p = str(df)
            ...
            try:
                ...
                raise TypeError("Failed to convert polars.DataFrame to pandas.DataFrame")
        elif not isinstance(df, _pd.DataFrame):
            raise TypeError("df must be a pandas.DataFrame, a polars.DataFrame, or a file path")

        if rename_map:
            # Allow user overrides/extension of the default renamer
            try:
                df = self.renamedf(df, rename_map=rename_map)
            except TypeError:
                # Back-compat if renamedf doesn't accept rename_map
                df = self.renamedf(df)

        if ts_col and ts_col in df.columns:
            try:
                df[ts_col] = _pd.to_datetime(df[ts_col], errors="coerce", utc=False)
            except Exception:
                pass
        df = self.renamedf(df)
        ...
        # Convert Sonic and Air Temperatures from Degrees C to Kelvin
        df["Ts"] = self.convert_CtoK(df["Ts_ro"])
        df = _assign(df, Ta=self.convert_CtoK(_get_series(df,"Ta")))
        ...
        for varib in ["Ux", "Uy", "Uz", "Q", "Ts", "Tsa"]:
            self.errvals[varib] = self.calc_MSE(df[varib])
        ...
        # Determine Wind Direction
        Ux_avg = df["Uxr"].mean().item()
        Uy_avg = df["Uyr"].mean().item()
        Uz_avg = df["Uzr"].mean().item()
    ...
    def webb_pearman_leuning(
        self,
        lamb: float | np.ndarray | pl.Series | pd.Series,
        Tsa: float | np.ndarray | pl.Series | pd.Series,
        pVavg: float,
        Uz_Ta: float,
        Uz_pV: float,
    ) -> float:
...
import pandas as pd  # optional compat, numpy as np
    ...
    def calc_MSE(self, y: pl.Series) -> float:
            """
            Compute the **mean square error** (MSE) of a one-dimensional series, i.e.
            ...
            ----------
            y : polars.Series
                One-dimensional Polars Series (numeric). No NaN handling is
            ...
            * This is **not** the unbiased sample variance (which divides by
            :math:`N – 1`); it is the population variance—also equal to the mean of the
            squared deviations.
            * The function is fully vectorized, leveraging Polars' high-performance engine.
            ...
            >>> from ec import CalcFlux
            >>> calc = CalcFlux()
            >>> arr = pl.Series([1.0, 2.0, 3.0, 4.0])
            >>> calc.calc_MSE(arr)
            1.25
            """
            # Calculate the square of the deviations (y - y.mean()).pow(2)
            # Then, compute the mean of these squared deviations.
            # .item() extracts the result as a standard Python scalar.
            return (y - y.mean()).pow(2).mean().item()

    def convert_KtoC(self, T: float | np.ndarray | pl.Series | pd.Series) -> float | np.ndarray | pl.Series | pd.Series:
        """
    ...
    def convert_CtoK(self, 
                     T: float | np.ndarray | pl.Series | pd.Series) -> float | np.ndarray | pl.Series | pd.Series:
        """
    ...
    def correct_KH20(
        self,
        Uz_Ta: float | np.ndarray | pl.Series,
        P: float | np.ndarray | pl.Series,
        T: float | np.ndarray | pl.Series,
    ) -> float | np.ndarray | pl.Series:
        """
    ...
    def correct_spectral(
        self,
        B: float | np.ndarray | pl.Series,
        alpha: float | np.ndarray | pl.Series,
        varib: float | np.ndarray | pl.Series,
    ) -> float | np.ndarray | pl.Series:
        """
    ...
    def get_Watts_to_H2O_conversion_factor(
        self,
        temperature: float | np.ndarray | pl.Series | pd.Series,
        day_fraction: float | np.ndarray | pl.Series | pd.Series,
    ) -> float | np.ndarray | pl.Series | pd.Series:
        """
    ...
    def calc_water_density(self, 
                           temperature: float | np.ndarray | pl.Series | pd.Series) -> float | np.ndarray | pl.Series | pd.Series:
        """
    ...
    def lamb_func(self, x: float | np.ndarray | pl.Series, varb: str) -> float | np.ndarray | pl.Series:
        """
    ...
    def calc_latent_heat_of_vaporization(
        self,
        temperature: float | pl.Series | pd.Series,
        units: str = "C",
    ) -> float | pl.Series | pd.Series:
        """
        ...
        Parameters
        ----------
        temperature : float or polars.Series
            Temperature at which :math:`\lambda` is required.
        units : {'C', 'K'}, default ``'C'``
        ...
        Returns
        -------
        float or polars.Series
            Latent heat :math:`\lambda` (:math:`\text{J kg}^{-1}`), preserving the
            input type (scalar → float, Series → Series).
        ...
        Notes
        -----
        * For :math:`T \ge 0^{\circ}\text{C}` the latent heat of **condensation/vaporisation** is
          returned (water phase).
        * For :math:`T < 0^{\circ}\text{C}` the latent heat of **sublimation** is returned (ice
          phase).
        * The underlying coefficients are identical to those in
        ...
        2.454
        >>> # Vectorised evaluation for ice phase
        >>> T_series = pl.Series("T", [-20.0, -10.0, -5.0])  # °C
        >>> calc.calc_latent_heat_of_vaporization(T_series).round(1)
        shape: (3,)
        Series: 'T' [f64]
        [
            2834100.0,
        ...
        """
        # Handle scalar float/int input explicitly to ensure float return type
        ...
        if is_scalar:
            if temperature >= 0:
                return self.lamb_func(temperature, "water")
            else:
                return self.lamb_func(temperature, "ice")

        # Polars vectorized implementation (assuming temperature is now a pl.Series)
        # Use pl.when/then/otherwise for conditional selection
        condition = temperature >= 0
        
        # The result doesn't need to be aliased here since the comparison is done 
        # outside of a frame expression, but for consistency in vectorized operations:
        series_out = (
            pl.when(condition)
            .then(self.lamb_func(temperature, "water"))
            .otherwise(self.lamb_func(temperature, "ice"))
        )
        
        # Preserve Series name if input was a named Series
        if hasattr(temperature, 'name') and temperature.name is not None:
             series_out = series_out.alias(temperature.name)
    ...
    def shadow_correction(self, Ux, Uy, Uz):
        ...
        * Original coefficients from Horst et al. (2015, *Atmos. Meas. Tech.*)
        ...
        return Uxc, Uyc, Uzc # type: ignore
...
import pandas as pd  # optional compat
    ...
    def calc_pV(
        self, Ea: float | np.ndarray | pd.Series | pl.Series, 
        Ts: float | np.ndarray | pd.Series | pl.Series,
    ) -> float | np.ndarray | pd.Series | pl.Series:
        """
